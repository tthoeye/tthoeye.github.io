---
layout: page
title: "Open Call: Interconnecting Local Digital Twins (Work Strand 1)"
permalink: /call-one/
---

# 🏙️ LDT4SSC Open Call: Interconnecting Local Digital Twins (Work Strand 1)

### Funding opportunity for smarter, connected communities across Europe

The **Local Digital Twins for Smart and Sustainable Communities (LDT4SSC)** project, funded by the **Digital Europe Programme (DEP)**, invites applications for its **first Open Call (Work Strand 1)**.

This call focuses on **interconnecting existing Local Digital Twins (LDTs)** to build a **federated European network** that supports seamless data exchange, shared services, and interoperability across communities.

The initiative advances the EU’s priorities in **digital innovation, sustainability, and ethical AI**, enabling regions and cities to collaborate, optimise urban services, and make data-driven decisions.

---

## 💡 About the First Open Call (Work Strand 1)

**Work Strand 1 (WS1)** supports pilot projects that demonstrate how existing Local Digital Twins can interconnect to form a European-scale network. Selected pilots will implement shared frameworks and services that promote interoperability and cross-domain collaboration.

**Funding available**
- Up to **€1,000,000 per consortium**  
- **50% co-funding required**  
- Maximum **€500,000 per third party** across all strands  
- Project duration: **12–18 months**

**Key dates**
- **Open Call launch:** 14 November 2025  
- **Submission deadline:** 15 January 2026, 23:59 CET  
- **Expected pilot start:** 14 May 2026  

**Eligibility**
This call targets **communities, regions, and organisations** operating existing data platforms or Local Digital Twin components, seeking to **scale through shared and interoperable frameworks.**

---

## 📄 Application Documents

To apply, please prepare and submit the following documents using the provided templates:

1. [**Application Form** – Download Template (.docx)](link-to-application-template)  
2. [**Budget Form** – Download Template (.xlsx)](link-to-budget-template)  
3. [**Consortium Declaration** – Download Template (.pdf)](link-to-consortium-template)  
4. [**Ethics & Data Management Statement** – Download Template (.docx)](link-to-ethics-template)  

All documents and the complete submission guidelines will be available from **14 November 2025** on this website.

---

## 📬 How to Apply

Applications must be submitted electronically via the **LDT4SSC Open Call Portal**:

👉 [**Submit Your Application**](link-to-submission-portal)

Before applying, please ensure:
- All required documents are completed and signed  
- The consortium meets the eligibility criteria  
- The proposal aligns with the objectives of Work Strand 1

---

## 👥 Become an Evaluator

Are you an expert in **digital twins, data spaces, or urban innovation**?  
We are also seeking **independent evaluators** for the LDT4SSC Calls for Pilots across all work strands.  
Compensation will be provided.

📄 [**Learn More & Apply as an Evaluator**](link-to-expert-call)

